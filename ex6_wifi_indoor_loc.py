...
import numpy as np
import os
train_data_path = 'data' +os.sep+'UJIndoorLoc'+os.sep+'trainingData.csv'
validation_data_path = 'data' +os.sep+'UJIndoorLoc'+os.sep+'validationData.csv'

training_data = pd.read_csv(train_data_path, header = 0)
...
test_dataset = pd.read_csv(validation_data_path, header = 0)
...
def train_neural_networks():
    ...
    with tf.Session() as sess:
        ...
        print("------------------------------------------------------------------")
        #先训练好autoencoder的encode部分,让解码器的输出尽量等于编码器的输入
        #之后监督学习利用DNN进行分类预测
