...
lex = create_lexicon(pos_file, neg_file)#lex里保存了文本中出现过的单词。
...
def normalize_dataset(lex):
    ...
    with open(pos_file, 'r') as f:
        lines = f.readlines()
        for line in lines:
            onesample = string_to_vector(lex, line, [1,0])
            dataset.append(onesample)
    with open(neg_file, 'r') as f:
        lines = f.readlines()
        for line in lines:
            onesample = string_to_vector(lex, line, [0,1])
            dataset.append(onesample)
    print "len_dataset:%d" % len(dataset)
    return dataset
...
#[None, len(train_x)]代表数据数据的高和宽（矩阵），好处是如果数据不符合宽高，tensorflow会报错，不指定也可以。, len(train_dataset[0][0])为features的长度
...
def train_neural_network(X, Y):
    ...
    with tf.Session() as session:
        ...
        print u'准确率: ' + str(accuracy.eval({X:list(text_x) , Y:list(text_y)}))
