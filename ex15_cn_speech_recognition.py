...
def get_wav_lable(wav_files=wav_files, label_file=label_file):
    ...
    with codecs.open(label_file, "r","utf-8") as f:
        for label in f:
            label = label.strip(u"\n")
            label_id = label.split(u" ", 1)[0]
            label_text = label.split(u" ", 1)[1]
...
print u"样本数:%d" % len(wav_files)  # 8911
...
print u"最长句子的字数:%d" % label_max_len

wav_max_len = 673
# for wav in wav_files:
#     wav, sr = librosa.load(wav, mono=True) #sr: sample rate, 采样频率
#     mfcc = np.transpose(librosa.feature.mfcc(wav, sr), [1,0])
#     if len(mfcc) > wav_max_len:
#         wav_max_len = len(mfcc)
print u"最长的语音的mfcc长度:%d" % wav_max_len

batch_size = 16
n_batch = len(wav_files) // batch_size

print u"n_batch: %d" % n_batch
