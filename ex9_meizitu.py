...
import numpy as np
dir_pre = 'data' + os.sep
old_dir = dir_pre + 'images'
new_dir = dir_pre + 'girls'
...
#             count += 1
# print u"Total Picture: %d" % count
#
#为了减小计算量，我把图像缩放为64×64像素
image_dir = new_dir
new_girl_dir = dir_pre + 'little_girls'
...
if MNIST == True:
    ...
    print u"图像总数: %d" % len(picture_list)
...
def gated_cnn(W_shape_, fan_in, gated=True, payload=None, mask=None, activation=True):
    ...
    def get_weights(shape, name, mask=None):
        ...
        W = tf.get_variable(name, shape, dtype=tf.float32, initializer=weights_initializer)
    ...
    if gated:
        ...
        b_f = tf.get_variable("v_b", b_shape, dtype=tf.float32, initializer=tf.zeros_initializer())
        b_g = tf.get_variable("h_b", b_shape, dtype=tf.float32, initializer=tf.zeros_initializer())
        ...
        b = tf.get_variable("b", b_shape, dtype=tf.float32, initializer=tf.zeros_initializer())
...
def pixel_cnn(layers=12, f_map=32):
    ...
    with tf.variable_scope("fc_2"):
        ...
        return fc2
model_path = 'model'+os.sep+"girl.ckpt"
epochs = 15
max_batch = 50
def train_pixel_cnn():
    output = pixel_cnn()
    ...
    with tf.Session(config = config) as sess:
        ...
        for epoch in range(epochs):
            for batch in range(max_batch):
                ...
                print u"epoch:%d  batch:%d  cost:%f" %( epoch, batch, cost)
            if epoch % 7 == 0:
...
def generate_girl():
    ...
    with tf.Session(config = config) as sess:
        ...
        saver = tf.train.Saver(tf.trainable_variables())
        saver.restore(sess, model_path+'-14')
